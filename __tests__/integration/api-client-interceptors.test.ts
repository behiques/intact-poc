...
// Mock the token store with all necessary methods
...
const setupStoreWithToken = (token: string, expiresAt: number) => {
  // Mock the token store to return the specified token
  mockGetValidToken.mockResolvedValue(token)

  // Update the store mock to return the correct state
  const mockUseTokenStore = vi.mocked(useTokenStore)
  vi.mocked(mockUseTokenStore.getState).mockReturnValue({
    token,
    expiresAt,
    isRefreshing: false,
    isLoading: false,
...
describe('API Client Interceptors Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Setup default mocks
    mockGetValidToken.mockRejectedValue(new Error('No token available'))
  ...
  describe('Request Interceptors', () => {
    it('should automatically attach token to requests', async () => {
      ...
      setupStoreWithToken(validToken, Date.now() + 3600000)
    ...
    it('should not attach expired token to requests', async () => {
      ...
      // Setup initialize to trigger token refresh when store has expired token
      mockInitialize.mockImplementation(async () => {
        if (mockHasValidStoredToken() === false) {
          await mockRetrieveToken()
        }
      })
  ...
  describe('Response Interceptors', () => {
    it('should automatically refresh token on 401 and retry request', async () => {
      ...
      setupStoreWithToken(expiredToken, Date.now() + 3600000)
  ...
  describe('Queue Management', () => {
    it('should queue concurrent requests during token refresh', async () => {
      ...
      mockFetch.mockImplementation(async (url: string) => {
        ...
        if (isRetry) {
          // Extract path from full URL for consistent testing
          ...
          return {
            ...
            json: vi
              .fn()
              .mockResolvedValue({ data: `success-${urlObj.pathname}` }),
            text: vi.fn().mockResolvedValue('Success'),
          }
        } else {
          return {
  ...
  describe('Error Boundaries', () => {
    ...
    it('should clear token on permanent authentication failure', async () => {
      ...
      // When clearToken is called, update the mock state to return null token
      mockClearToken.mockImplementation(() => {
        const mockUseTokenStore = vi.mocked(useTokenStore)
        vi.mocked(mockUseTokenStore.getState).mockReturnValue({
          token: null,
